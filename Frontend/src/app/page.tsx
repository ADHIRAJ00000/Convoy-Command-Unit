...
import { useEffect, useRef, useState, type RefObject, Suspense } from 'react';
...
const Hero3DBackground = dynamic(() => import('@/components/Hero3DBackground'), {
  ssr: false,
  loading: () => <div className="absolute inset-0 bg-gradient-to-b from-panelNight/40 via-panelNight/20 to-slateDepth" />,
});
  ...
  return (
    <div className="bg-slateDepth text-textNeutral min-h-screen">
      <main className="relative overflow-hidden">
        ...
        >
          {/* 3D Background */}
          <Suspense fallback={<div className="absolute inset-0 bg-gradient-to-b from-panelNight/40 via-panelNight/20 to-slateDepth" />}>
            <Hero3DBackground />
          </Suspense>

          {/* Gradient Overlays for Depth */}
          <motion.div
            className="absolute inset-0 bg-gradient-to-b from-slateDepth/30 via-transparent to-slateDepth/90 pointer-events-none"
            style={{ y: heroBackgroundY }}
            aria-hidden="true"
          />
          <motion.div
            className="absolute inset-0 bg-[radial-gradient(circle_at_center,_rgba(251,191,36,0.08),_transparent_70%)] pointer-events-none"
            style={{ y: heroGlowY }}
            aria-hidden="true"
          />
            ...
            >
              ...
              <div className="flex flex-wrap gap-4">
                ...
                >
                  <Link
                    href="/dashboard"
                    className="inline-block rounded-full bg-amberCommand px-6 py-3 text-sm font-semibold uppercase tracking-wide text-black shadow-command transition-all duration-200 hover:shadow-lg hover:shadow-amberCommand/50"
                  >
                    Enter Dashboard
                  </Link>
                </motion.div>
                ...
                >
                  <a
                    ...
                    className="inline-block rounded-full border border-textNeutral/30 px-6 py-3 text-sm font-semibold text-textNeutral transition-all duration-200 hover:border-amberCommand/40"
                  >
                    View Solution Stack
                  </a>
                </motion.div>
